"""
Password Guessing Game - Google ADK + Ryumem (Asynchronous Version)

This example demonstrates how Ryumem's query augmentation helps an agent learn from
previous attempts to guess a password more intelligently, using async execution.
...
Differences from Sync Version:
    • Uses runner.run_async() instead of runner.run()
    • Event iteration is async (async for event in event_stream)
    • Better for production environments with concurrent requests
...
# App configuration
APP_NAME = "password_guessing_game_async"
USER_ID = "player_async_1"
SESSION_ID = "game_session_async_1"
...
async def main():
    """Main function to run the password guessing game with async execution."""

    print("=" * 80)
    print("🔐 Password Guessing Game - Google ADK + Ryumem (Async Version)")
    print("=" * 80)
    print()
    print("GAME RULES:")
    ...
    print()
    print("ASYNC VERSION BENEFITS:")
    print("  • Non-blocking execution using run_async()")
    print("  • Better for concurrent requests in production")
    print("  • Async event iteration with 'async for'")
    print("=" * 80)
    print()
...
If the user wants a hint, use get_hint.""",
    ...
    memory = add_memory_to_agent(
        password_agent,
        user_id=USER_ID,
        ryumem_customer_id="password_game_async",
    )
    ...
    for query_idx, query in enumerate(queries, 1):
        ...
        try:
            # 🔥 KEY: Using run_async instead of run
    ...
    print("   This version uses run_async() for non-blocking execution.")
    print("   Perfect for production environments with concurrent user requests.")
